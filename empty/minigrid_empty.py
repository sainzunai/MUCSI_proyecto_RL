...
env_str = "MiniGrid-Empty-Random-6x6-v0"
load_model_str = 'MiniGrid-Empty-Random-5x5-v0'
algo = "dqn"
...
if continue_training:
    #model = PPO.load("models/{}_{}".format(algo, load_model_str), env=env, tensorboard_log="./logs/{}".format(env_str))
    model = DQN.load("models/{}_{}".format(algo, load_model_str), env=env, tensorboard_log="./logs/{}".format(env_str))
else:
    #model = PPO("MlpPolicy", env, verbose=1, tensorboard_log="./logs/{}".format(env_str))
    model = DQN("MlpPolicy", env, verbose=1, tensorboard_log="./logs/{}".format(env_str))

# Train the agent and display a progress bar
model.learn(total_timesteps=int(2e6), reset_num_timesteps=(False)) 
# Save the agent
